...
def clear_outtime_ssr():
    """
    删除过时的ssr，超过6次检测超时，则认为已经失效
    """
    mc = MongoClient(**config['mongo'])
    mc.server_info()
    ssr_db = mc['proxy']
    ssr_coll = ssr_db['ss_config']
    ssr_coll.delete_many({'latency': {'$gte': 9.99}, 'check_cnt': {'$gte': 26}})
...
def get_latency_of_proxy(ssr_config, process='0'):
    ...
    def getaddrinfo(*args):
        # 原理为原样返回 [(af, socktype, proto, canonname, sa)]
...
if __name__ == "__main__":
    ...
    # join 之前的check_cnt
    for one in result:
        key_str = '{}:{}'.format(one['server'], one['server_port'])
        one['check_cnt'] = config['ssr_config'][key_str]['check_cnt'] if key_str in config['ssr_config'] else 0
    ...
    result = sorted(config['ssr_config'].values(), key=lambda x: x['latency'])
    for one_result in result[:1]:
        pts = PrettyTable(["server_url"] + list(one_result.keys()))
        for one_result in result[:5]:
            server_url = ':'.join([one_result['server'], str(one_result['server_port'])])
            pts.add_row([server_url] + list(one_result.values()))
        logging.info("\n{}".format(pts.get_string(fields=[
            "remarks", "latency", "server_url", "password", "method"
        ])))
